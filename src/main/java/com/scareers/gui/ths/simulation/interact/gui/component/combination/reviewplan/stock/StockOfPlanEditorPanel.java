 ...
 */
@Getter
public class StockOfPlanEditorPanel extends DisplayPanel {
    ...
    public StockOfPlanEditorPanel(StockPanelForPlan parentPanel) {
    ...
    private void initOther() {
        ...
        StockOfPlanEditorPanel panelTemp = this;
...
//                    dialog.setLocation();
    ...
    private JPanel initLeaderStockListEditPanel() {
        ...
        // 1.左列表: 全部成分股
        Vector<ThsDbApi.ThsSimpleStock> allIncludeStockList = new Vector<>();
        ...
        // 3.右列表: 龙头股
        Vector<ThsDbApi.ThsSimpleStock> leaderStocks = new Vector<>();
...
//                bean.updateLeaderStockList(stocks);
     ...
     * @param bean
     */
    public void update(StockOfPlan bean) {
        this.bean = bean;
    ...
    public StockOfPlan getEditedBean() {
        if (this.bean == null) {
            return null;
        }
...
//        bean.setBadAspects(badAspectsValueLabel.getText());
    ...
    private static KeyAdapter buildKeyAdapterForEdit(StockOfPlanEditorPanel panel) {
     ...
     * @return
     */
    private static FocusListener buildJTextFieldBlurForEdit(StockOfPlanEditorPanel panel) {
    ...
    public static void tryAutoSaveEditedBean(StockOfPlanEditorPanel panel, String logPrefix) {
        StockOfPlan editedBean = panel.getEditedBean();
...
//        jTextField.addFocusListener(buildJTextFieldBlurForEdit(panel));
     ...
     * @return
     */
    private static KeyListener buildKeyAdapterForTrendEdit(StockOfPlanEditorPanel panel) {
        return new KeyAdapter() {
            @Override
            public void keyPressed(KeyEvent e) { // 按下回车, 自动保存当前bean. null时忽略
                if (e.getKeyCode() == KeyEvent.VK_ENTER) {
     ...
     * @param panel
     */
    public static void flushWithRelatedTrends(StockOfPlanEditorPanel panel) {
...
//        ManiLog.put("StockOfPlan: 已自动计算并保存: 关联行业概念 trend 加成折算因子");
    ...
    public static JXComboBox getCommonJXComboBox(
            StockOfPlanEditorPanel panel, Vector<?> items) {
