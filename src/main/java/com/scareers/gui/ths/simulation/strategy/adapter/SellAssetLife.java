...
public class SellAssetLife {
    ...
    // 1. 高卖分布仓位算法基本参数默认值
    /**
     * 高卖分布仓位算法简介: 以最简单情况, next0(明日)买入, next1(后日)尝试高卖
     ...
     * 1.决策: 当股票价格==+0.01时(<平均值), 我们的剩余仓位应当极大, 已卖出仓位应当偏小, 因为我们还有极大可能, 价格能够达到 +0.03附近;
     * 2.决策: 当股票价格==+0.03时(约=平均值), 我们的剩余仓位应当约等于一半?(存疑), 因为我们大约有一半概率, 价格能够达到 >+0.03更高;
     * 3.决策: 当股票价格==+0.05时(>平均值), 我们的剩余仓位应当极小, 已卖出仓位应当极大, 因为我们仅有极小可能, 价格能够达到>0.05;
     * 4.显然, 常态情况下, 单只股票的多个"卖点", 若前面的卖点价格更高,而后面的卖点价格更小, 则后面的卖点无效,
     * 因为理论上后面的卖点对应的 "总已卖出仓位" 应当更小. 然而覆水难收,
     ...
     * 6.某股票出现若干个高点卖点后, 将有一个max价格;
     * 在此后某一个时间点, 若我们认为在这个时间点的这个价格后(此价格低于已发生的max),基本上 极难 再恢复到 > 原max价格了,
     * 此时应当无条件全部卖出剩余仓位!
     * 原则: 对于每一个时刻, 估算 "未来将有价格>已出现max" 的概率, 若此概率接近0, 则应立即清仓该股票. 若此概率接近100%, 则应0卖出.
     * 原则: "未来将有价格>已出现max" 依然是一个随机变量, 该变量分布应当初步与 "当前价格/已max回落百分比" 和 "今日剩余交易时间" 两大因素相关;
     * 决策: 当 current_price >= max_already 时, 适用 1/2/3 的情况,
     * 决策: 当 current_price < max_already, 应计算 未来将有价格>已出现max 事件的概率 P = f(max_already, fall_percent, remain_time_today)
     *
     *
     * 另外: @notification
     */
