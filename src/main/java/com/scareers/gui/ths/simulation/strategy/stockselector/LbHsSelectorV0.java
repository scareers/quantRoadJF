...
/**
 * description: 基于原始研究结果的低买高卖 选股方法 -- V0
 *
 * @author: admin
 * @date: 2021/12/26/026-03:21:08
 */
public class LbHsSelectorV0 extends LbHsSelector {
    public static void main(String[] args) throws Exception {
        LbHsSelectorV0 lbHsSelectorV0 = new LbHsSelectorV0(Arrays.asList(), 10, false, Arrays.asList(0, 1));
        lbHsSelectorV0.selectStock();
        lbHsSelectorV0.selectStock();
        lbHsSelectorV0.resetSelectResults();
        lbHsSelectorV0.selectStock();

        Console.log(lbHsSelectorV0.getSelectResults());
        Console.log(lbHsSelectorV0.getCdfOfHighSell().size());
        Console.log(lbHsSelectorV0.getCdfOfLowBuy().size());
    }
    ...
    private boolean preferenceMoreStock;
...
//    private List<Double> ticksOfLowBuy = null; // [0.11, 0.105, 0.1, 0.095, 0.09, 0.085, 0.08, 0.075, ...
//    private List<Double> weightsOfLowBuy = null; // 44数据
//    private List<Double> ticksOfHighSell = null; // [-0.215, -0.21, -0.205, -0.2, -0.195, -0.19, -0.185, ..
//    private List<Double> weightsOfHighSell = null; // 88数据
    ...
    public LbHsSelectorV0(List<String> forceManualExcludeStocks, // 需要设置手动排除的股票.
    ...
    public void selectStock() throws Exception {
        if (getSelectResults() != null) {
            log.warn("LbHsSelectorV0 已经执行选股, 不可再次执行, 请调用 getSelectResults() 获取结果");
            return;
        }
    ...
    private void initFinalDistributionPdf() throws Exception {
        ...
        // 更新全局分布!!!
        ticksOfLowBuy = ticksOfLow1Global; // [0.11, 0.105, 0.1, 0.095, 0.09, 0.085, 0.08, 0.075, ...
        weightsOfLowBuy = weightsOfLow1Global; // 44数据
        ticksOfHighSell = ticksOfHigh1Global; // [-0.215, -0.21, -0.205, -0.2, -0.195, -0.19, -0.185, ..
        weightsOfHighSell = weightsOfHigh1Global; // 88数据
        cdfOfLowBuy = CommonUtil.cumSumOfListDouble(weightsOfLowBuy);
        cdfOfHighSell = CommonUtil.cumSumOfListDouble(weightsOfHighSell);


        Assert.isTrue(sumEqualApproximately(weightsOfLowBuy, 1.0, 0.001));//权重和1
        Assert.isTrue(sumEqualApproximately(weightsOfHighSell, 1.0, 0.001));//权重和1
        log.info("show: 低买tick: {}", ticksOfLowBuy);
        log.info("show: 低买分布: {}", weightsOfLowBuy);
        log.info("show: 高卖tick: {}", ticksOfHighSell);
        log.info("show: 高卖分布: {}", weightsOfHighSell);
    }
...
//                log.info("formSetId no stockSelectResult: 无选股结果: {}", forSetId);
    ...
    /**
     * 单日实际选股逻辑, 该方法会对所有主板股票进行选股逻辑, 将全部结果保存.
     *
    ...
    private void stockSelect0() throws Exception {
        ...
        log.warn("stock all select calc finish: 全股票今日选股主逻辑完成, 尚未筛选. keyInts: {}", keyInts);
    }
