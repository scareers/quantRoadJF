...
public class SettingsOfFSBacktest {
    ...
    public static Double positionUpperLimit = 1.5; // 控制上限, 一般不大于 倍率, 当然, 这些倍率都是对于 1只股票1块钱而言
    // @noti: 这些限制设定, 应当 / totalAsserts, 才能等价
    public static Double positionCalcKeyArgsOfCdf = 1.5; // 控制单股cdf倍率, 一般不小于上限
    public static final Double execLowBuyThreshold = -0.02; // 必须某个值 <= -0.1阈值, 才可能执行低买, 否则跳过不考虑
    ...
    public static final Double execHighSellThreshold = 0.005; // 必须 >0.01阈值, 才可能执行高卖,
    public static int continuousRaiseTickCountThreshold = 2; // 高卖时, 连续上升数量的阈值, 应当不小于这个数量, 才考虑卖. 1最宽容,可考虑2,包含相等
    // 连接对象
